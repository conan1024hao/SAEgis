...
def parse_args():
    ...
    parser.add_argument(
        "--ssa-cwa-feature-path",
        default=None,
        help="Path to SSA-CWA top_features.json",
    )
    parser.add_argument(
        "--m-attack-feature-path",
        default=None,
        help="Path to M-Attack top_features.json",
    )
    parser.add_argument(
        "--foa-attack-feature-path",
        default=None,
        help="Path to FOA-Attack top_features.json",
    )
...
def infer_dataset_and_sae(feature_path):
    # Expected shape: .../<dataset>/<sae_location>/top_features.json
    p = Path(feature_path)
    if len(p.parts) >= 3:
...
def main():
    args = parse_args()
    dataset_A = json.load(open(args.ssa_cwa_feature_path, "r"))["top_1000_features"]
    dataset_B = json.load(open(args.m_attack_feature_path, "r"))["top_1000_features"]
    dataset_C = json.load(open(args.foa_attack_feature_path, "r"))["top_1000_features"]

    topk = args.topk
    inferred_dataset, inferred_sae = infer_dataset_and_sae(args.ssa_cwa_feature_path)
    dataset = args.dataset or inferred_dataset
    sae_location = args.sae_location or inferred_sae

    default_filename = f"top_{topk}_feature_overlap_venn_{dataset}__{sae_location}.png"
    ...
    venn3(
        ...
        set_labels=("SSA-CWA", "M-Attack", "FOA-Attack"),
    )
